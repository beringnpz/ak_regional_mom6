...
latlim = minmax(Grd.geolat_c);
lonlim = minmax(wrapTo360(Grd.geolon_c));

% Fill defaults where necessary

if isempty(slat)
    slat = mean(Grd.geolat(:));
end
if isempty(slon)
    slon = mean(Grd.geolon(:));
end

% Check data size

[nxi, neta] = size(Grd.geolon);
sz = size(bhis);
if ndims(bhis) > 3 || ~isequal(sz(1:2), [nxi neta])
    error('bhis must be nxh x nyh x nt array');
end
...
if isempty(bsta)
    idx = knnsearch([Grd.geolat(:) Grd.geolon(:)], [slat slon], 'Distance', distfun);
    [ixi,ieta] = ind2sub(size(Grd.geolat), idx);
    nstat = length(slat);
...
h.pc   = pcolorm(double(Grd.geolat_c), double(Grd.geolon_c), double(padend(bhis(:,:,1))));
...
set(h.fig, 'DeleteFcn', @closefig);
    ...
    function updatetime(t)
        ...
        set(h.pc, 'cdata', double(padend(bhis(:,:,ihis))));
